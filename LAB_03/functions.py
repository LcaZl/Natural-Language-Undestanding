...
class MyStupidBackoff(LanguageModel):
    ...
    """
    def __init__(self, alpha=0.4, epsilon = 1e-3, *args, **kwargs):
        """
        Initializes my Stupid Backoff model.

        Parameters:
        - alpha (float, optional): The backoff discount factor. Default is 0.4.
        - epsilon (float, optional): Constant to avoid zero probabilities.
    ...
    def backoff(self, ngram):
        ...
        if len(ngram) == 1:
            ...
            return (self.counts[ngram[0]] + self.epsilon) / total_unigrams
        ...
        if ngram_freq > 0:
            ...
            reduced_alpha = self.alpha / 2.0  # Reduce alpha
            return reduced_alpha * self.backoff(ngram[1:]) + self.epsilon
...
def train_model(model, training_set, ngram_order, alpha = None, epsilon = None, discount = None, gamma = None):
    """
...
def dataset_info(dataset, dataset_name, ngram_len, vocab_cutoff):
    ...
    # Get ngram counter
    padded_ngram_dataset, _ = padded_everygram_pipeline(ngram_len, dataset)
    ngram_counter = NgramCounter(padded_ngram_dataset)
